      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16}}>
          ...
          {pg==="staffview"&&isMgr&&<div style={cd}>
            ...
            {schMode==="edit"&&<div style={{fontSize:11,color:A,marginBottom:8}}>點選格子指派班次 · 再點可取消</div>}
            {schMode==="edit"&&<div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>{cfg.shifts.map(sh=><span key={sh.id} style={{display:"flex",alignItems:"center",gap:4,fontSize:10}}><span style={{width:10,height:10,borderRadius:2,background:sh.bg,border:`0.5px solid ${sh.bd}`}}/>{sh.label}</span>)}</div>}
              ...
              <tbody>{act.map(emp=><tr key={emp.id}><td style={{padding:"5px 8px",border:"0.5px solid var(--color-border-tertiary)",whiteSpace:"nowrap",position:"sticky",left:0,background:"var(--color-background-primary)",zIndex:2}}><span style={tg(emp.type==="正"?"#E6F1FB":AL,emp.type==="正"?"#0C447C":AD)}>{emp.type}</span> {emp.name}</td>
                {Array.from({length:D}).map((_,i)=>{const d=i+1,isH=hols.includes(d),isR=rests[emp.id+"-"+d];
                  ...
                  return<td key={d} style={{padding:0,border:"0.5px solid var(--color-border-tertiary)",minWidth:32,verticalAlign:"top",position:"relative"}}>
                    {isH?<div style={{padding:2,textAlign:"center",background:"var(--color-background-secondary)",minHeight:24}}><span style={{fontSize:8,color:"var(--color-text-tertiary)"}}>休</span></div>:
                    <div onClick={()=>setActiveCell(isActive?null:{empId:emp.id,day:d})} style={{padding:1,minHeight:24,cursor:"pointer",background:isActive?"#EEEDFE":"transparent"}}>
                      {dayShifts.length>0?dayShifts.map((a,ai)=>{const sh=cfg.shifts.find(x=>x.id===a.shift);return sh?<div key={ai} style={{fontSize:7,padding:"1px 2px",borderRadius:2,background:sh.bg,color:sh.fg,marginBottom:1,lineHeight:1.2,textAlign:"center"}}>{sh.label.slice(0,2)}</div>:null;})
                        :<div style={{textAlign:"center",fontSize:8,color:"var(--color-text-tertiary)",lineHeight:"24px"}}>+</div>}
                    </div>}
