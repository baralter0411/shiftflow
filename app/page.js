...
export default function Page(){
  ...
  function exportScheduleXls(){
    const hdr=["日期","星期","班別",...act.map(s=>s.name)];
    const rows=[];
    for(let d=1;d<=D;d++){
      if(hols.includes(d)){rows.push([`${mo+1}/${d}`,DW[gd(yr,mo,d)],"公休",...act.map(()=>"")]);continue;}
      const w=gd(yr,mo,d);const shifts=cfg.shifts.filter(sh=>sh.dow.includes(w));const sc=sch[d]||[];
      shifts.forEach((sh,i)=>{const r=[i===0?`${mo+1}/${d}`:"",i===0?DW[w]:"",sh.label];act.forEach(st=>r.push(sc.some(a=>a.id===st.id&&a.shift===sh.id)?"●":""));rows.push(r);});
    }
    const ws=XLSX.utils.aoa_to_sheet([hdr,...rows]);
    const wb=XLSX.utils.book_new();
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16}}>
          ...
          {pg==="staffview"&&isMgr&&<div style={cd}>
            ...
            {schMode==="edit"&&<div style={{fontSize:11,color:A,marginBottom:8}}>點選格子指派班次 · 再點可取消 · <span style={{color:"#A32D2D"}}>▲ 紅角標記 = 員工排休</span></div>}
              ...
              <tbody>{act.map(emp=><tr key={emp.id}><td style={{padding:"5px 8px",border:"0.5px solid var(--color-border-tertiary)",whiteSpace:"nowrap",position:"sticky",left:0,background:"var(--color-background-primary)",zIndex:2}}><span style={tg(emp.type==="正"?"#E6F1FB":AL,emp.type==="正"?"#0C447C":AD)}>{emp.type}</span> {emp.name}</td>
                {Array.from({length:D}).map((_,i)=>{const d=i+1,isH=hols.includes(d),isR=rests[emp.id+"-"+d];
                  ...
                  return<td key={d} style={{padding:0,border:"0.5px solid var(--color-border-tertiary)",minWidth:32,verticalAlign:"top",position:"relative"}}>
                    ...
                    {isActive&&!isH&&<div style={{position:"absolute",top:"100%",left:0,zIndex:10,background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-secondary)",borderRadius:6,padding:3,minWidth:70,boxShadow:"0 2px 8px rgba(0,0,0,.15)"}}>
                      ...
                      {isR&&<div style={{padding:"3px 6px",fontSize:10,color:"#A32D2D",borderTop:"0.5px solid var(--color-border-tertiary)",marginTop:2,paddingTop:4}}>員工想休息</div>}
                    </div>}
                  </td>;
                })}</tr>)}</tbody></table></div>
          </div>}
